...
export class AuthService {
  ...
  async confirmEmail(hash: string): Promise<void> {
    ...
    try {
      ...
      user_id = jwtData.confirmEmailuser_id;
    } catch {
      throw new UnprocessableEntityException({
        status: HttpStatus.UNPROCESSABLE_ENTITY,
        errors: {
  ...
  async resetPassword(hash: string, password: string): Promise<void> {
    ...
    await this.sessionService.deleteByuser_id({
      user_id: user.id,
    });
  ...
  async update(userJwtPayload: JwtPayloadType, userDto: AuthUpdateDto): Promise<NullableType<User>> {
    ...
    if (userDto.password) {
      ...
      if (!isValidOldPassword) {
        ...
        await this.sessionService.deleteByuser_idWithExclude({
    ...
    if (userDto.email && userDto.email !== currentUser.email) {
      ...
      const hash = await this.jwtService.signAsync(
        {
          confirmEmailuser_id: currentUser.id,
